...
class MiniBatch:
    def __init__(self, voxceleb, batch_size):
        ...
        self.voxceleb_batch = pd.DataFrame(pd.concat([anchor_batch, positive_batch, negative_batch], axis=0))
        self.audio_loaded = False
        self.num_triplets = num_triplets

    def load_wav(self):
        self.voxceleb_batch['raw_audio'] = self.voxceleb_batch['filename'].apply(lambda x: read_audio(x))
        logging.info(self.voxceleb_batch.head(10))
        min_existing_frames = min(self.voxceleb_batch['raw_audio'].apply(lambda x: len(x)).values)
        ...
        self.voxceleb_batch['raw_audio'] = self.voxceleb_batch['raw_audio'].apply(lambda x: x[start_frame:end_frame])
    ...
    def to_inputs(self):
        ...
        x = self.voxceleb_batch['raw_audio'].values
        new_x = []
        for sig in x:
            new_x.append(pre_process_inputs(sig, target_sample_rate=SAMPLE_RATE))
        x = np.array(new_x)
        y = self.voxceleb_batch['speaker_id'].values
...
def stochastic_mini_batch(voxceleb, batch_size):
    mini_batch = MiniBatch(voxceleb, batch_size)
    return mini_batch


def main():
    from voxceleb_wav_reader import read_voxceleb_structure
    voxceleb = read_voxceleb_structure(c.DATASET_DIR)
    stochastic_mini_batch(voxceleb, 3)
