...
def read_voxceleb_structure(directory):
    ...
    voxceleb['filename'] = find_files(directory)
    voxceleb['filename'] = voxceleb['filename'].apply(lambda x: x.replace('\\', '/')) # normalize windows paths
    voxceleb['utterance_id'] = voxceleb['filename'].apply(lambda x: x.split('/')[-1].split('.')[0])
    voxceleb['video_id'] = voxceleb['filename'].apply(lambda x: x.split('/')[-2])
    voxceleb['speaker_id'] = voxceleb['filename'].apply(lambda x: x.split('/')[-3])
...
def test():
    voxceleb = read_voxceleb_structure('C:/Users/Jin/Desktop/wav')
    read_audio(voxceleb[0:1]['filename'].values[0])
